def is_safe_report(report):
    ...
    # If not strictly increasing or strictly decreasing, return False
    if not (increasing or decreasing):
        return False
    
    # Check the difference condition (difference between adjacent levels should be between 1 and 3)
    for i in range(len(levels)-1):
        diff = abs(levels[i] - levels[i+1])
        if diff < 1 or diff > 3:
            return False
    
    # If all conditions are satisfied, it's a safe report
    return True
...
# Call the function to count safe reports
